...
function confirmMC(item) {
  ...
  if (!item.answer) return // 无答案则不做判分
        ...
        <div v-for="item in typeItems" :key="item.id" class="card">
          ...
          <template v-else-if="item.type === 'multiple_choice'">
            ...
            <div v-if="getMCState(item.id).showAnswer" style="margin-top:8px;font-weight:600">
              <template v-if="getMCFeedback(item.id)">
                <span v-if="getMCFeedback(item.id).isCorrect" class="fb-correct">🎉 正确！</span>
                <span v-else class="fb-wrong">
                  😞 错误！正确答案：
                  <template v-for="(a, ai) in getMCFeedback(item.id).correctAnswers" :key="a">
